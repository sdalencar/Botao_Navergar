 ...
 * made by sda
 */
public class AddBonusDao {

    private CriarBancoDados helper;
    private Context ctx;
    private SQLiteDatabase db;

    public AddBonusDao(Context ctx) {
        this.ctx = ctx;
        helper = new CriarBancoDados(ctx);
    }
    //abrir DB
    public AddBonusDao openDB() {
        try {
    ...
    public long adiciona(String identificador, String nome, String relacao, double valor) {
        try {
            ...
            return db.insert(Constantes.TABELA_BONUS, Constantes.ID, cv);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return 0;
    ...
    public Cursor buscar() {
        ...
        return db.query(Constantes.TABELA_BONUS, columns, null, null, null, null, null);
    }
    ...
    public long atualiza(int id, String identificador, String nome, String relacao, double valor) {
        try {
            ...
            cv.put(Constantes.RELACIONAMENTO, relacao);
            cv.put(Constantes.VALOR, valor);
            return db.update(Constantes.TABELA_BONUS, cv, Constantes.ID + " =?", new String[]{String.valueOf(id)});
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return 0;
    ...
    public long apagar(int id) {
        try {
            return db.delete(Constantes.TABELA_BONUS, Constantes.ID + " =?", new String[]{String.valueOf(id)});
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return 0;
